...
// https://docs.streamprotocol.app/docs/develop/node/intro/types-of-node
const config = {
    keyStore,
    networkId: "testnet",
    nodeUrl: "https://archival-rpc.testnet.streamprotocol.app",
};
...
async function getTransactions(startBlock, endBlock, accountId) {
    ...
    const blockDetails = await Promise.all(
        blockArr.map((blockId) =>
            stream.connection.provider.block({
                blockId,
            })
        )
    );
    ...
    const chunkDetails = await Promise.all(
        chunkHashArr.map(chunk => stream.connection.provider.chunk(chunk))
    );
    ...
    const txsLinks = transactions.map((txs) => ({
        ...
        link: `https://explorer.testnet.streamprotocol.app/transactions/${txs.hash}`,
    }));
...
async function getBlockByID(blockID) {
    const stream = await connect(config);
    const blockInfoByHeight = await stream.connection.provider.block({
        blockId: blockID,
    });
