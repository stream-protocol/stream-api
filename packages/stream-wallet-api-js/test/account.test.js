...
beforeAll(async() => {
    ...
    workingAccount = await testUtils.createAccount(streamjs);
});
...
test('create account and then view account returns the created account', async() => {
    ...
    const newAccount = new Account(streamjs.connection, newAccountName);
    const state = await newAccount.state();
    expect(state.amount).toEqual(newAmount.toString());
});

test('send money', async() => {
    const sender = await testUtils.createAccount(streamjs);
    const receiver = await testUtils.createAccount(streamjs);
...
describe('with deploy contract', () => {
    ...
    beforeAll(async() => {
        const newPublicKey = await streamjs.connection.signer.createKey(contractId, testUtils.networkId);
    ...
    test('view contract state', async() => {
        ...
        const contractAccount = await streamjs.account(contractId);
    ...
    test('get total stake balance and validator responses', async() => {
        ...
        const account = new Account(mockConnection, 'test.stream');
