...
describe('rpc-errors', () => {
    test('test AccountAlreadyExists error', async() => {
        let rpc_error = {
            TxExecutionError: {
                ActionError: {
                    index: 1,
                    kind: { AccountAlreadyExists: { account_id: 'bob.stream' } }
                }
            }
        };
        ...
        expect(error.account_id).toBe('bob.stream');
        expect(formatError(error.type, error)).toBe('Can\'t create a new account bob.stream, because it already exists');
    });
    ...
    test('test ReceiverMismatch error', async() => {
            ...
            TxExecutionError: {
                InvalidTxError: {
                    InvalidAccessKeyError: {
                        ReceiverMismatch: {
                            ak_receiver: 'test.stream',
                            tx_receiver: 'bob.stream'
                        }
                    }
                }
            }
        ...
        expect(error.ak_receiver).toBe('test.stream');
        expect(error.tx_receiver).toBe('bob.stream');
        expect(formatError(error.type, error)).toBe(
            'Wrong AccessKey used for transaction: transaction is sent to receiver_id=bob.stream, but is signed with function call access key that restricted to only use with receiver_id=test.stream. Either change receiver_id in your transaction or switch to use a FullAccessKey.'
        );
    });
    ...
    test('test getErrorTypeFromErrorMessage', () => {
        const err1 = 'account random.stream does not exist while viewing';
    ...
    test('test NotEnoughBalance message uses human readable values', () => {
        const error = parseRpcError({
            NotEnoughBalance: {
                ...
                signer_id: 'test.stream'
            }
        });

        expect(error.message).toEqual('Sender test.stream does not have enough balance 1 for operation costing 10');
    });

    test('test TriesToStake message uses human readable values', () => {
        const error = parseRpcError({
            TriesToStake: {
                account_id: 'test.stream',
        ...
        expect(error.message).toEqual('Account test.stream tried to stake 10, but has staked 1 and only has 9');
    });
});