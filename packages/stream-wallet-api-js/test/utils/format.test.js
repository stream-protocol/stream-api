...
` ('formatStreamAmount($balance, $fracDigits) returns $expected', ({ balance, fracDigits, expected }) => {
    expect(streamApi.utils.format.formatStreamAmount(balance, fracDigits)).toEqual(expected);
});
...
` ('parseStreamAmount($amt) returns $expected', ({ amt, expected }) => {
    expect(streamApi.utils.format.parseStreamAmount(amt)).toStrictEqual(expected);
});

test('parseStreamAmount fails when parsing values with ≥25 decimal places', () => {
    expect(() => {
        streamApi.utils.format.parseStreamAmount('0.0000080990999998370878871');
    }).toThrowError(
        'Cannot parse \'0.0000080990999998370878871\' as STREAM amount'
    );
});