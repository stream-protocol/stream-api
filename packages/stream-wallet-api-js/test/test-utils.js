...
const HELLO_WASM_PATH = process.env.HELLO_WASM_PATH || 'node_modules/stream-hello/dist/main.wasm';
...
async function setUpTestConnection() {
    ...
    if (config.masterAccount) {
        ...
        await keyStore.setKey(networkId, config.masterAccount, streamApi.utils.KeyPair.fromString('ed25519:2wyRcSwSuHtRVmkMCGjPwnzZmQLeXLzLLyED1NDMt4BjnKgQL6tF85yBx6Jr26D2dUNeC716RBoTxntVHsegogYw'));
    }
    return streamApi.connect(config);
}
...
async function createAccount(stream) {
    const newAccountName = generateUniqueString('test');
    const newPublicKey = await stream.connection.signer.createKey(newAccountName, networkId);
    await stream.createAccount(newAccountName, newPublicKey);
    const account = new streamApi.Account(stream.connection, newAccountName);
    return account;
}

async function createAccountMultisig(stream, options) {
    const newAccountName = generateUniqueString('test');
    const newPublicKey = await stream.connection.signer.createKey(newAccountName, networkId);
    await stream.createAccount(newAccountName, newPublicKey);
    ...
    try {
        const confirmKeyPair = streamApi.utils.KeyPair.fromRandom('ed25519');
        const { publicKey } = confirmKeyPair;
        // const account = new streamApi.Account(stream.connection, newAccountName);
        // await account.addKey(publicKey, account.accountId, streamApi.multisig.MULTISIG_CONFIRM_METHODS, '0')
        // create multisig account instance and deploy contract
        const accountMultisig = new streamApi.multisig.AccountMultisig(stream.connection, newAccountName, options);
        accountMultisig.useConfirmKey = async() => {
            await stream.connection.signer.setKey(networkId, options.masterAccount, confirmKeyPair);
        };
...
async function deployContract(workingAccount, contractId) {
    ...
    return new streamApi.Contract(workingAccount, contractId, HELLO_WASM_METHODS);
}
