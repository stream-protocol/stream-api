...
// If an access key has itself as receiverId and method permission add_request_and_confirm, then it is being used in a wallet with multisig contract: https://github.com/stream/core-contracts/blob/671c05f09abecabe7a7e58efe942550a35fc3292/multisig/src/lib.rs#L149-L153
...
let keyStore = new streamApi.keyStores.InMemoryKeyStore();
beforeEach(() => {
    keyStore.clear();
    streamFake = {
        config: {
        ...
        connection: {
            networkId: 'networkId',
            signer: new streamApi.InMemorySigner(keyStore)
        },
        account() {
            return {
                state() {}
    ...
    walletConnection = new streamApi.WalletConnection(streamFake);
});
...
describe('fails gracefully on the server side (without window)', () => {
    ...
    it('does not throw on instantiation', () => {
        expect(() => new streamApi.WalletConnection(streamFake)).not.toThrowError();
    });

    it('returns an empty string as accountId', () => {
        const serverWalletConnection = new streamApi.WalletConnection(streamFake);
...
it('can complete sign in', async() => {
    const keyPair = streamApi.KeyPair.fromRandom('ed25519');
    global.window.location.href = `http://example.com/location?account_id=stream.account&public_key=${keyPair.publicKey}`;
    await keyStore.setKey('networkId', 'pending_key' + keyPair.publicKey, keyPair);

    await walletConnection._completeSignInWithAccessKey();

    expect(await keyStore.getKey('networkId', 'stream.account')).toEqual(keyPair);
...
it('Promise until complete sign in', async() => {
    const keyPair = streamApi.KeyPair.fromRandom('ed25519');
    global.window.location.href = `http://example.com/location?account_id=stream2.account&public_key=${keyPair.publicKey}`;
    await keyStore.setKey('networkId', 'pending_key' + keyPair.publicKey, keyPair);

    const newWalletConn = new streamApi.WalletConnection(streamFake, 'promise_on_complete_signin');

    expect(newWalletConn.isSignedIn()).toEqual(false);
    expect(await newWalletConn.isSignedInAsync()).toEqual(true);
    expect(await keyStore.getKey('networkId', 'stream2.account')).toEqual(keyPair);
...
const blockHash = streamApi.utils.serialize.base_decode(BLOCK_HASH);
...
function parseTransactionsFromUrl(urlToParse, callbackUrl = 'http://example.com/location') {
    ...
    const transactions = parsedUrl.query.transactions.split(',')
        .map(txBase64 => streamApi.utils.serialize.deserialize(
            streamApi.transactions.SCHEMA,
            streamApi.transactions.Transaction,
...
function setupWalletConnectionForSigning({ allKeys, accountAccessKeys }) {
    walletConnection._authData = {
        allKeys: allKeys,
        accountId: 'signer.stream'
    };
    streamFake.connection.provider = {
        query(params) {
            if (params.request_type === 'view_account' && params.account_id === 'signer.stream') {
                return {};
            }
            if (params.request_type === 'view_access_key_list' && params.account_id === 'signer.stream') {
                return { keys: accountAccessKeys };
            }
            if (params.request_type === 'view_access_key' && params.account_id === 'signer.stream') {
    ...
    it('V2', async() => {
        try {
            await walletConnection.account().signAndSendTransaction({
                receiverId: 'receiver.stream',
                actions: [streamApi.transactions.transfer(1)],
...
describe('requests transaction signing automatically when function call has attached deposit', () => {
    beforeEach(async() => {
        const localKeyPair = streamApi.KeyPair.fromRandom('ed25519');
        const walletKeyPair = streamApi.KeyPair.fromRandom('ed25519');
        setupWalletConnectionForSigning({
        ...
        await keyStore.setKey('networkId', 'signer.stream', localKeyPair);
    });
    ...
    it('V2', async() => {
        try {
            await walletConnection.account().signAndSendTransaction({
                receiverId: 'receiver.stream',
                actions: [streamApi.transactions.functionCall('someMethod', new Uint8Array(), new BN('1'), new BN('1'))],
...
describe('requests transaction signing with 2fa access key', () => {
    beforeEach(async() => {
        let localKeyPair = streamApi.KeyPair.fromRandom('ed25519');
        let walletKeyPair = streamApi.KeyPair.fromRandom('ed25519');
        setupWalletConnectionForSigning({
            allKeys: [walletKeyPair.publicKey.toString()],
            accountAccessKeys: [{
                access_key: {
...
describe('fails requests transaction signing without 2fa access key', () => {
    ...
    it('V2', () => {
        ...
        ).rejects.toThrow('Cannot find matching key for transaction sent to receiver.stream');
    });
});
...
describe('can sign transaction locally when function call has no attached deposit', () => {
    ...
    it.each([
        streamApi.transactions.functionCall('someMethod', new Uint8Array(), new BN('1'), new BN('0')),
        streamApi.transactions.functionCall('someMethod', new Uint8Array(), new BN('1')),
        streamApi.transactions.functionCall('someMethod', new Uint8Array())
