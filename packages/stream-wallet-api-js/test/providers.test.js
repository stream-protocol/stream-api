...
const withProvider = (fn) => {
    ...
    const provider = new streamApi.providers.JsonRpcProvider(config.nodeUrl);
    return () => fn(provider);
};
...
test('json rpc fetch block info', withProvider(async(provider) => {
    ...
    let streamFinalBlock = await provider.block({ finality: 'stream-final' });
    expect(streamFinalBlock.header.height - height).toBeLessThan(5);
...
test('txStatus with string hash and buffer hash', withProvider(async(provider) => {
    const stream = await testUtils.setUpTestConnection();
    const sender = await testUtils.createAccount(stream);
    const receiver = await testUtils.createAccount(stream);
...
test('json rpc query account', withProvider(async(provider) => {
    const stream = await testUtils.setUpTestConnection();
    const account = await testUtils.createAccount(stream);
...
test('final tx result', async() => {
    ...
    expect(streamApi.providers.getTransactionLastResult(result)).toEqual({});
});
...
test('final tx result with null', async() => {
    ...
    expect(streamApi.providers.getTransactionLastResult(result)).toEqual(null);
});

test('json rpc light client proof', async() => {
    const stream = await testUtils.setUpTestConnection();
    const workingAccount = await testUtils.createAccount(stream);
    const executionOutcome = await workingAccount.sendMoney(workingAccount.accountId, new BN(10000));
    const provider = stream.connection.provider;
...
test('JsonRpc connection object exist without connectionInfo provided', async() => {
    const provider = new streamApi.providers.JsonRpcProvider();
    expect(provider.connection).toStrictEqual({ url: '' });
});

test('stream json rpc fetch node status', async() => {
    const config = require('./config')(process.env.NODE_ENV || 'test');
    const stream = await streamApi.connect(config);
    let response = await stream.connection.provider.status();
    expect(response.chain_id).toBeTruthy();
});