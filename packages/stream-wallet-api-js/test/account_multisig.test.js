...
const {
    ...
    utils: { format: { parseStreamAmount } }
} = streamApi;
...
const getAccount2FA = async(account, keyMapping = ({ public_key: publicKey }) => ({ publicKey, kind: 'phone' })) => {
    ...
    const account2fa = new Account2FA(streamjs.connection, accountId, {
        ...
        onAddRequestResult: async() => {
            ...
            const originalSigner = streamjs.connection.signer;
            streamjs.connection.signer = await InMemorySigner.fromKeyPair(streamjs.connection.networkId, accountId, account2fa.confirmKey);
            ...
            streamjs.connection.signer = originalSigner;
        }
    });
...
beforeAll(async() => {
    streamjs = await testUtils.setUpTestConnection();
    let nodeStatus = await streamjs.connection.provider.status();
...
describe('deployMultisig key rotations', () => {
    ...
    test('full access key if recovery method is "ledger" or "phrase", limited access key if "phone"', async() => {
        const account = await testUtils.createAccount(streamjs);
...
describe('account2fa transactions', () => {
    ...
    test('add app key before deployMultisig', async() => {
        let account = await testUtils.createAccount(streamjs);
        ...
        await account.addKey(appPublicKey.toString(), appAccountId, appMethodNames, new BN(parseStreamAmount('0.25')));
    ...
    test('send money', async() => {
        let sender = await testUtils.createAccount(streamjs);
        let receiver = await testUtils.createAccount(streamjs);
        sender = await getAccount2FA(sender);
        receiver = await getAccount2FA(receiver);
        const { amount: receiverAmount } = await receiver.state();
        await sender.sendMoney(receiver.accountId, new BN(parseStreamAmount('1')));
        const state = await receiver.state();
        expect(BigInt(state.amount)).toBeGreaterThanOrEqual(BigInt(new BN(receiverAmount).add(new BN(parseStreamAmount('0.9'))).toString()));
    });
    ...
    test('send money through signAndSendTransaction', async() => {
        ...
        receiver = await getAccount2FA(receiver);
        const { amount: receiverAmount } = await receiver.state();
        await sender.signAndSendTransaction({ receiverId: receiver.accountId, actions: [transfer(new BN(parseStreamAmount('1')))] });
        const state = await receiver.state();
        expect(BigInt(state.amount)).toBeGreaterThanOrEqual(BigInt(new BN(receiverAmount).add(new BN(parseStreamAmount('0.9'))).toString()));
    });
