...
test('make function call using access key', async() => {
    const keyPair = streamApi.utils.KeyPair.fromRandom('ed25519');
    ...
    await streamjs.connection.signer.keyStore.setKey(testUtils.networkId, workingAccount.accountId, keyPair);
...
test('view account details after adding access keys', async() => {
    ...
    const keyPair2 = streamApi.utils.KeyPair.fromRandom('ed25519');
...
test('load invalid key pair', async() => {
    ...
    await streamjs.connection.signer.keyStore.setKey(testUtils.networkId, workingAccount.accountId, '');
    try {
        await contract.setValue({ args: { value: 'test' } });
        fail('should throw an error');
    } catch (e) {
        expect(e.message).toEqual(`no matching key pair found in ${streamjs.connection.signer}`);
